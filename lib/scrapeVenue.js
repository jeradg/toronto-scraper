    ...
    $row.find( 'td' ).each( function( column ) {
      if ( $( this ).find( '.coursetitlecol' ).length > 0 ) {
        ...
        if ( ( $( this ).find( 'br' ) ) && ( $( this ).html().trim().split( /<br>|<br \/>/ ).length > 1 ) ) {
          ...
          var sessions = $( this ).html().trim().split( /<br>|<br \/>/ );
          ...
          var sessions = [ $( this ).text().trim() ];
        }

        if ( sessions ) {
          for ( var i = 0; i < sessions.length; i++ ) {
            var session = {},
                times = dateHelpers.processTimes( dates[ column - 1 ], sessions[ i ] );
  ...
  function processBody( err, resp, body ) {
    if ( err ) {
      ...
      if ( $( '[id^="dropin_Swimming"]' ).length < 1 ) {
        ...
        var error = new Error( 'No drop-in swimming programmes detected for url ' + url );

        callback( error );
      } else {
        // The regex removes newlines and tabs from the venue name.
        var venue = {
              name: cleanVenueName( $( '#pfrContentBody h1' ).text() ),
        ...
        request.get( geocoderURLEscaped, function( err, resp, body ) {
          if ( err ) {
            ...
            venue.normalizedUrl = venue.name.replace( /[-]/g, ' ' ) // Turn hyphens into spaces
            ...
            if ( venue.name === 'Joseph J. Piccininni Community Centre' ) {
              venue.coordinates = [ -79.451007, 43.675246 ];
            } else if ( venue.name === 'Giovanni Caboto' ) {
              venue.coordinates = [ -79.451858, 43.675638 ];
            } else {
              venue.coordinates = [ longitude, latitude ];
            }
            ...
            venue.ward = parseInt( $( '.pfrComplexLocation ul li:contains("Ward:")' ).text().replace( /Ward: /gm, '' ), 10 );
