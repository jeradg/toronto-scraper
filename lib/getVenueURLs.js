var exports = module.exports = getVenueURLs,
    cheerio = require( 'cheerio' ),
    request = require( 'request' ),
    async = require( 'async' );

function getVenueURLs( indexURLs, callback ) {
  var venueURLs = [];
  ...
  if ( !Array.isArray( indexURLs ) ) {
    var error = new Error( 'first argument of getVenueURLs must be an array of URLs' );

    callback( error );
  // Check if the array has at least one member.
  } else if ( indexURLs.length < 1 ) {
    var error = new Error( 'first argument of getVenueURLs must be an array with at least one URL' );
