...
function aggregateAges( activities, callback ) {
  VenueModel.aggregate( [ {
    ...
    return createMetadata( activities, ages, callback );
  } );
}

function createMetadata( activities, ages, callback ) {
  MetadataModel.create( { activities: activities, ages: ages }, function( error, metadata ) {
    if ( !error ) {
      callback();
    } else {
