var exports = module.exports = requestURL,
    ...
    Activity = require( './constructors/activity' ),
    Week = require( './constructors/week' );
  ...
  request( url, function( error, resp, body ) {
    if ( !error ) {
      ...
      if ( $( '[id^="dropin_Swimming"]' ).length > 0 ) {
        ...
        thisVenue.name = $( '#content-body h1' ).text()
          .replace( /(\r\n|\n|\r|\t)/gm, '' )
          .replace( /([ \t\r\n][ \t\r\n]*)/, ' ' )
          .replace( / And /g, ' and ' ).trim();

        if ( thisVenue.name === 'North Toronto Memorial Community' ) {
          thisVenue.name = 'North Toronto Memorial Community Centre'
        } else if ( thisVenue.name === 'Centennial Recreation Centre-Scarborough' ) {
          thisVenue.name = 'Centennial Recreation Centre - Scarborough'
        }
        } else if ( thisVenue.name === 'West Mall Outdoor Pool - Outdoor Pool' ) {
          thisVenue.name = 'West Mall Outdoor Pool'
        }
