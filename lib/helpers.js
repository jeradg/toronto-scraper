...
exports.processTimes = function( dateObject, rawTimeRange, activity ) {
  var thisSession = {},
...
exports.callbackCounter = function( counterObject, condition, callback, arguments ) {
  // The counter needs to be an object because objects are passed as arguments by reference,
  // which means you can update them from within the function. Numbers are passed by
  // value, so you can't update the original variable by updating the argument inside the
  // function.
  if ( counterObject.counter === condition - 1 ) {
    callback( arguments );
  }
